...
export const archivePlaylists =
	(spotify: SpotifyWebApiNode) => (userId: string, date: string) =>
		spotify
					...
					await spotify
						...
						.then((tracks) => {
							...
							writeFile(
								...
								() =>
									console.log(
										`Archived "${playlist.name}" with ${trackData.length} items`,
									),
							);
						})
						.catch((e) =>
							console.log(`Error while archiving: "${playlist.name}"`, e),
						);
				}
			});
